...
export function swapPage(page, scope = "root") {
  if (!(page instanceof Node)) {
    console.warn("SwapPage: page must be a DOM node", page);
    return;
  }

  const target = resolveScope(scope);
  if (!target) {
    console.warn("SwapPage: scope not found", scope);
    return;
  }
...
export function addPage(page, scope = "root") {
  if (!(page instanceof Node)) {
    console.warn("addPage: page must be a DOM node", page);
    return;
  }

  const target = resolveScope(scope);
  if (!target) {
    console.warn("addPage: scope not found", scope);
    return;
  }
